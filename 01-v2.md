...
| `image_seed`      | `opaque image_seed[32]`                 | Exactly 32 bytes   | ChaCha20-Poly1305 seed for deriving the encryption key for the group image. MUST be cryptographically random when generated. Also serves as the seed for deriving the Nostr keypair used for Blossom upload authentication (see [Image Upload Identity](#image-upload-identity)). May be all zeros if no image is set. |
| `image_nonce`    | `opaque image_nonce[12]`               | Exactly 12 bytes   | ChaCha20-Poly1305 nonce for group image encryption. MUST be cryptographically random and unique for each image. Used with `image_seed` for authenticated encryption/decryption. May be all zeros if no image is set. |
...
```
image_seed = random(32)          // Cryptographically random key
image_nonce = random(12)        // Cryptographically random nonce

image_key = HKDF-Expand(image_seed, "mip01-image-encryption-v2", 32)
encrypted_image = ChaCha20-Poly1305.encrypt(image_key, image_nonce, image_data)
```

**Upload Authentication**: The Nostr keypair for Blossom upload authentication MUST be deterministically derived from the `image_seed`:

```
upload_secret = HKDF-Expand(image_seed, "mip01-blossom-upload-v2", 32)
upload_keypair = secp256k1_keypair_from_secret(upload_secret)
```

This derivation provides domain separation between the encryption key and the signing key, while ensuring that anyone with the `image_seed` can derive the upload keypair for cleanup purposes.

**Update and Cleanup Workflow**:

When an admin updates the group image:

1. **Generate new keys**: Create new random `image_seed` (32 bytes) and `image_nonce` (12 bytes)
2. **Encrypt new image**: Use ChaCha20-Poly1305 to encrypt the new group image
3. **Derive upload keypair**: Compute `upload_secret = HKDF-Expand(new_image_seed, "mip01-blossom-upload-v2", 32)` and derive the Nostr keypair
...
5. **Update extension**: Commit an MLS proposal updating `image_hash`, `image_seed`, and `image_nonce` fields
6. **Cleanup old image**: After successful commit, derive the previous upload keypair from `old_image_seed` and request deletion of the old image blob from Blossom using the derived keypair for authentication
...
- **Cleanup capability**: Any admin who has access to a previous `image_seed` can derive the upload keypair to delete that image blob
...
function validate_structure(data, version) {
    ...
    //               admins(2+0) + relays(2+0) + image_hash(32) + image_seed(32) + image_nonce(12)
    ...
    REMAINING_FIXED = 32 + 32 + 12  // image_hash + image_seed + image_nonce
    return (offset + REMAINING_FIXED == data.length)
}
```

**Forward Compatibility**: When encountering unknown future versions:
- Parse known fields in order (version, nostr_group_id, name, description, admin_pubkeys, relays, image_hash, image_seed, image_nonce)
...
- Deterministic upload keypair derivation using `HKDF-Expand(image_seed, "mip01-blossom-upload-v1", 32)` for Blossom authentication and cleanup

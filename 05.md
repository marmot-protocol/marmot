...
**Tags**:
- **token**: One or more `token` tags with five elements in this order:
  - Second element: Complete encrypted token as base64
  - Third element: Notification server's public key (hex)
  - Fourth element: A relay hint URL where the notification server's `kind: 10050` event can be found
  - Fifth element: MLS leaf index (decimal string) of the device that owns this token

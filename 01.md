...
| `image_key`      | `opaque image_key[32]`                 | Exactly 32 bytes   | ChaCha20-Poly1305 seed for deriving the encryption key for the group image. MUST be cryptographically random when generated. Also serves as the seed for deriving the Nostr keypair used for Blossom upload authentication (see [Image Upload Identity](#image-upload-identity)). May be all zeros if no image is set. |
...
image_encryption_key = HKDF-Expand(image_key, "mip01-image-encryption-v2", 32)
encrypted_image = ChaCha20-Poly1305.encrypt(image_encryption_key, image_nonce, image_data)
```
...
```
upload_secret = HKDF-Expand(image_key, "mip01-blossom-upload-v2", 32)
...
3. **Derive upload keypair**: Compute `upload_secret = HKDF-Expand(new_image_key, "mip01-blossom-upload-v2", 32)` and derive the Nostr keypair
...
Version 2 introduces a breaking change to how the `image_key` field is used for group image encryption. This change improves cryptographic hygiene by ensuring proper domain separation between the encryption key and the upload authentication key.

### Key Differences

| Aspect | Version 1 | Version 2 |
| ------ | --------- | --------- |
| `image_key` purpose | Direct encryption key | Seed for key derivation |
| Image encryption | `ChaCha20-Poly1305.encrypt(image_key, ...)` | `ChaCha20-Poly1305.encrypt(HKDF-Expand(image_key, "mip01-image-encryption-v2", 32), ...)` |
| Upload auth derivation | `HKDF-Expand(image_key, "mip01-blossom-upload-v1", 32)` | `HKDF-Expand(image_key, "mip01-blossom-upload-v2", 32)` |

### Migration Notes

- **Not backward compatible**: Version 2 clients cannot decrypt images encrypted by version 1 clients (and vice versa) without knowing which version was used
- **New groups**: MUST use version 2 format
- **Existing groups**: Groups created with version 1 will continue to work but cannot interoperate with version 2 clients for image decryption. Clients should strive to upgrade groups to the new version where possible.

### Rationale

In version 1, the `image_key` served dual purposes: it was used directly as the ChaCha20-Poly1305 encryption key, and it was also used as input to HKDF for deriving the upload authentication keypair. Version 2 applies HKDF derivation consistently for both purposes, providing:

1. **Consistent key derivation**: Both the encryption key and upload key are now derived from the seed using HKDF with distinct info strings
2. **Domain separation**: Clear cryptographic separation between encryption and authentication contexts
3. **Future flexibility**: The seed-based approach allows for additional derived keys if needed in future versions
...
- Deterministic key derivation using `HKDF-Expand(image_key, "mip01-image-encryption-v2", 32)` for image encryption and `HKDF-Expand(image_key, "mip01-blossom-upload-v2", 32)` for Blossom authentication and cleanup

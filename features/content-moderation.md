...
Deleting a chat message does not erase report explanations that quote it; kind 4891 targets chat messages only.
...
An admin dismisses a report using a [NIP-32](https://github.com/nostr-protocol/nips/blob/master/32.md) label event. Its tags
MUST include `["L", "marmot.report-review.v1"]`, `["l", "dismissed", "marmot.report-review.v1"]`, and one or more `e` tags
referencing kind-1984 report events in the same group. Its `content` may explain the dismissal, as in NIP-32.
Each `e` tag MUST begin `["e", report_id]`, with a 64-character lowercase hexadecimal id; a malformed reference has no
effect for that reference. Trailing tag elements and additional labels do not change the dismissal meaning. Without
the required `L` and `l` tag prefixes, the event MUST have no dismissal effect.
...
Clients MUST retain sufficient evidence of established authorization for as long as its effects are needed. A recorded
verdict bound to the event and its authenticated branch is sufficient; pruning source-epoch state MUST NOT undo an
authorized dismissal or deletion.
...
A missing referenced report MUST remain unresolved and be reevaluated when it arrives, subject to existing retention
rules.

...
```json
{
  ...
  "content": "v1:MDk4NzY1NDMyMWZlZGNiYS4uLg==",
  "tags": [
...
**Required fields:**
- **`content`**: Serialized `MLSMessage` containing the MLS `Welcome` object, encoded in one of two formats:
  - **Version 1 (base64)**: Prefix `v1:` followed by base64-encoded content (e.g., `v1:SGVsbG8=`). This is the preferred format for reduced payload size (~33% smaller).
  - **Legacy (hex)**: No prefix, hex-encoded content. This format is supported for backward compatibility but SHOULD NOT be used for new Welcome Events.

  The version tag eliminates ambiguity for strings like `deadbeef` that are valid in both hex and base64 formats but decode to completely different bytes.
...
The `content` field contains a version-tagged base64-encoded (or legacy hex-encoded) MLS `Welcome` object that includes:
